...
There are `r nrow(mm10.IAP)` IAP (sub)element entries, representing `r length(unique(mm10.IAP$element.ID))` IAP elements. These elements are made up of `r length(unique(mm10.IAP$repName))` IAP subelement types:
*`r paste(unique(mm10.IAP$repName), sep=", ")`*.

Within an IAP, the subelements may be missing components (relative to their full-length reference), or be broken up by other retroelements that have internaled themselves within the element. Note that many of the subelements which appear multiple times in the same IAP are often actually fragmented parts of a full subelement.
...
```{r refine.IAP.structure, include=FALSE, eval=FALSE, cache=TRUE}
REFINE.MAX.GAP <- 0
...
A faster and more digestible solution was to simply 'collapse' adjacent subelements of the same type into a single subelement of that type (e.g., `IAPLTR1a_Mm|IAPEz-int|IAPEz-int|IAPLTR1a_Mm` becomes `IAPLTR1a_Mm|IAPEz-int|IAPLTR1a_Mm`).
...
_NB: in the list above and plot below, element names are missing the 'IAP' prefix for brevity_
...
![example](data/misc/example_broken_IAP.PNG)

To fix these, I will look for at elements of the annotation of the three patterns above, and try to stitch them together. For example, if an `LTR || internal` element is followed by an `internal || LTR` element nearby (or even a solo LTR), I will combine them.
...
mm10.IAP.mended <- distinct(as.data.frame(mcols(mm10.IAP.gr)[,c("orig.element.ID", "element.ID")]))
```

This process resulted in `r with(mm10.IAP.mended, sum(orig.element.ID != element.ID))` elements being combined with other elements, and increased the number of fully-structured IAPs from `r sum(mm10.IAP.reduced.old$fully.structured)` to `r sum(mm10.IAP.reduced$fully.structured)`. *fix this*

An example of the mended vs old (RepeatMasker) annotation is shown below.
...
What kind of IAP element structure do we see at MEs? How does this compare with the whole genome and with false positives? 

** Need to run next section; maybe in new file? either way absolute paths need to be fixed, and interpretation below checked **

```{r structure.of.MEs, eval=FALSE}
mm10.IAP.top_name.structure <- mm10.IAP.gr %>% as.data.frame() %>%
...
* ~~Genome-wide, many elements are just internal portions (have they lost all transpostion ability without flanking LTRs?)~~ Internal-only IAP elements have been largely eliminated by merging them with surrounding LTRs.
* Genome-wide, there are also many solo LTRs (especially of the `IAPEY2_LTR` and `IAPLTR2a2_Mm` type)
* MEs are mostly either fully-structured (of type `LTR1 || Z-internal || LTR1`), or solo LTRs of type `IAPLTR2_Mm`
* False positives are enriched for relatively 'rare' structures, including solo, fully-structured, and partially-structured `IAPEY4` elements (although the N is very low for these, perhaps this says something at the `EY4` type elements and their susceptibility to poor mapping?
