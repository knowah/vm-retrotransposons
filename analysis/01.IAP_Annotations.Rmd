...
Within an IAP, the subelements may be missing components (relative to their full-length reference), or be broken up by other retroelements that have internaled themselves within the element.

An example of these annotations is below: 
...
```{r obtain.element.seq, eval=FALSE}
# 
...
```{r refine.IAP.structure, eval=FALSE, cache=TRUE}
REFINE.MAX.GAP <- 0
...
Many IAPs are of a 'partial' structure in the annotation: i.e., `LTR || internal` or solo `internal`s or `internal || LTR`.
...
![example](example_broken_IAP.PNG).
